"""Copies QC_Journal from 1C into the target DB (60-day rolling window)."""
import sys
import os
import uuid
...
from sql import QUERY_QC_JOURNAL_WINDOW_TEMPLATE

WINDOW_DAYS   = 60
TABLE_STAGING = "Import_1C.stg_QC_Journal"
...
def _ensure_snapshot_id(cur_t):
    cur_t.execute("""
        ...
        WHERE TableName = 'Import_1C.QC_Journal'
    """)
    row = cur_t.fetchone()
    ...
    cur_t.execute("SELECT TOP (1) SnapshotID FROM Import_1C.QC_Journal")
    row = cur_t.fetchone()
    if row and row[0] is not None:
        existing = str(row[0])
        cur_t.execute("""
            MERGE Import_1C.SnapshotPointer AS t
            USING (SELECT 'Import_1C.QC_Journal' AS TableName) s
...
class QCJournalCopy(BaseMigration):
    script_id        = "1c_qc_journal"
    script_name      = "1C QC Journal (60-day window)"
    interval_seconds = 60
    category         = "continuous"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            query = QUERY_QC_JOURNAL_WINDOW_TEMPLATE.format(
            ...
            self.acquire_applock(cur_t, "Migration_QC_Journal")
            cur_t.execute(
                "DELETE FROM Import_1C.QC_Journal WHERE SnapshotID = ? AND [Date] BETWEEN ? AND ?;",
                (snapshot_id, date_from_real, date_to_real)
            )

            cur_t.execute(
                """
                EXEC Import_1C.sp_SwitchSnapshot_QC_Journal
                  @SnapshotID = ?, @DateFrom = ?, @DateTo = ?,
                  @Full = 0, @CleanupPrev = 1;
                """,
                (snapshot_id, date_from_real, date_to_real)
            )
            conn_t.commit()

            cur_t.execute("EXEC QC.sp_Refresh_LQC_Journal")
            conn_t.commit()

            cur_t.execute("SELECT COUNT(*) FROM Import_1C.vw_QC_Journal_Current")
