"""QC Journal Full Sync — complete dump from 1C, runs weekly (Sunday 03:40)."""
import sys
import os
...
class QCJournalFullSync(BaseMigration):
    script_id   = "1c_qc_journal_full"
    script_name = "QC Journal Full Sync (1C)"
    category    = "scheduled"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            cur_t.execute(
                "EXEC Import_1C.sp_SwitchSnapshot_QC_Journal @SnapshotID=?, @Full=1, @CleanupPrev=1",
                (snapshot_id,)
            )
            conn_t.commit()
...
if __name__ == "__main__":
    QCJournalFullSync().run_once_standalone()
