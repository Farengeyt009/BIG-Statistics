"""Materials Move Full Sync — complete dump from 1C, runs weekly (Sunday 03:20)."""
import sys
import os
...
# Full range in 1C date format: 4025-01-01 → today+2000
...
class MaterialsMoveFullSync(BaseMigration):
    script_id   = "1c_materials_move_full"
    script_name = "Materials Move Full Sync (1C)"
    category    = "scheduled"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            idx_date = columns_1c.index('Doc_Date') if 'Doc_Date' in columns_1c else None
            shifted = []
            for row in rows_1c:
                row = list(row)
            ...
            cur_t.execute(
                "EXEC Import_1C.sp_SwitchSnapshot_Materials_Move @SnapshotID=?, @Full=1, @CleanupPrev=1",
                (snapshot_id,)
            )
            conn_t.commit()
...
if __name__ == "__main__":
    MaterialsMoveFullSync().run_once_standalone()
