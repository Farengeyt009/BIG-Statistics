"""Price List Full Sync — complete dump from 1C, runs weekly (Sunday 04:40)."""
import sys
import os
...
class PriceListFullSync(BaseMigration):
    script_id   = "1c_price_list_full"
    script_name = "Price List Full Sync (1C)"
    category    = "scheduled"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            cur_t.execute(
                "EXEC Import_1C.sp_SwitchSnapshot_Price_List @SnapshotID=?, @Full=1, @CleanupPrev=1",
                (snapshot_id,)
            )
            conn_t.commit()
...
if __name__ == "__main__":
    PriceListFullSync().run_once_standalone()
