"""Copies Price_List from 1C into the target DB (365-day rolling window)."""
import sys
import os
import uuid
...
from sql import QUERY_PRICE_LIST_WINDOW_TEMPLATE

WINDOW_DAYS   = 365
TABLE_STAGING = "Import_1C.stg_Price_List"
...
def _ensure_snapshot_id(cur_t):
    cur_t.execute("""
        ...
        WHERE TableName = 'Import_1C.Price_List'
    """)
    row = cur_t.fetchone()
    ...
    cur_t.execute("SELECT TOP (1) SnapshotID FROM Import_1C.Price_List")
    row = cur_t.fetchone()
    if row and row[0] is not None:
        existing = str(row[0])
        cur_t.execute("""
            MERGE Import_1C.SnapshotPointer AS t
            USING (SELECT 'Import_1C.Price_List' AS TableName) s
...
class PriceListCopy(BaseMigration):
    script_id        = "1c_price_list"
    script_name      = "1C Price List (365-day window)"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            date_to_4025   = dt_date(year=date_to_real.year   + 2000, month=date_to_real.month,   day=date_to_real.day)
            date_from_4025 = dt_date(year=date_from_real.year + 2000, month=date_from_real.month, day=date_from_real.day)

            query = QUERY_PRICE_LIST_WINDOW_TEMPLATE.format(
                date_from=date_from_4025.strftime("%Y-%m-%d"),
                date_to=date_to_4025.strftime("%Y-%m-%d")
            )
            ...
            if 'Date' in columns_1c:
                idx_date = columns_1c.index('Date')
                prepped = []
                for row in rows_1c:
                    row = list(row)
                    if row[idx_date] is not None:
            ...
            self.acquire_applock(cur_t, "Migration_Price_List")
            cur_t.execute(
                """
            ...
            cur_t.execute(
                """
                EXEC Import_1C.sp_SwitchSnapshot_Price_List
                  @SnapshotID  = ?,
                  @DateFrom    = ?,
                  @DateTo      = ?,
                  @Full        = 0,
            ...
            cur_t.execute("SELECT COUNT(*) FROM Import_1C.vw_Price_List_Current")
