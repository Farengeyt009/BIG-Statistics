"""Copies Outsource_Price from 1C into the target DB (365-day rolling window)."""
import sys
import os
import uuid
...
from sql import QUERY_OUTSOURCE_PRICE_WINDOW_TEMPLATE
...
def _ensure_snapshot_id(cur_t):
    cur_t.execute("""
        ...
        WHERE TableName = 'Import_1C.Outsource_Price'
    """)
    row = cur_t.fetchone()
    ...
    cur_t.execute("SELECT TOP (1) SnapshotID FROM Import_1C.Outsource_Price")
    row = cur_t.fetchone()
    if row and row[0] is not None:
        existing = str(row[0])
        cur_t.execute("""
            MERGE Import_1C.SnapshotPointer AS t
            USING (SELECT 'Import_1C.Outsource_Price' AS TableName) s
...
class OutsourcePriceCopy(BaseMigration):
    script_id        = "1c_outsource_price"
    script_name      = "1C Outsource Price (365-day window)"
    ...
    def run_once(self) -> int:
        ...
        try:
            ...
            query = QUERY_OUTSOURCE_PRICE_WINDOW_TEMPLATE.format(
            ...
            cur_t.execute(
                """
                EXEC Import_1C.sp_SwitchSnapshot_Outsource_Price
                  @SnapshotID = ?,
                  @DateFrom   = ?,
                  @DateTo     = ?,
                  @Full       = 0,
            ...
            cur_t.execute("SELECT COUNT(*) FROM Import_1C.vw_Outsource_Price_Current")
