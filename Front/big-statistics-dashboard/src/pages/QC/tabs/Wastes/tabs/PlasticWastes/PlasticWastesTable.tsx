...
import '../StampingWastes/ag-grid-overrides.css';
...
const NUM_FIELDS = [
  'FACT_QTY', 'WeightTotal_FACT', 'Cost_FACT',
  'QCCard_Others_QTY', 'WeightOthers', 'CostOthers',
  'WeightWastes_FACT', 'CostWastes_FACT',
  'Debugging_QTY', 'WeightDebugging', 'CostDebugging',
  'Weight_Total', 'Weight_Wastes', 'GP_Weight', 'Price',
];
...
const PlasticWastesTable: React.FC<Props> = ({ data, loading, error, suppressLocalLoaders }) => {
  ...
  const columnDefs: ColDef[] = useMemo(() => applyStandardFilters([
    ...
    numCol('WeightTotal_FACT',   t('wastes.columns.weightFact'),        140),
    costCol('Cost_FACT',         t('wastes.columns.costFact'),          140),
    numCol('WeightWastes_FACT',  t('wastes.columns.weightWastesFact'),  200),
    costCol('CostWastes_FACT',   t('wastes.columns.costWastesFact'),    200),
    numCol('QCCard_Others_QTY',  t('wastes.columns.othersQty'),         140),
    numCol('WeightOthers',       t('wastes.columns.weightOthers'),      155),
    costCol('CostOthers',        t('wastes.columns.costOthers'),        140),
    numCol('Debugging_QTY',      t('wastes.columns.debuggingQty'),      155),
    numCol('WeightDebugging',    t('wastes.columns.weightDebugging'),   155),
    costCol('CostDebugging',     t('wastes.columns.costDebugging'),     140),
    numCol('Weight_Total',       t('wastes.columns.weightTotal'),       140),
    numCol('Weight_Wastes',      t('wastes.columns.weightWastes'),      140),
    numCol('GP_Weight',          t('wastes.columns.gpWeight'),          140),
  ...
  const actionsSlot = typeof document !== 'undefined'
    ? document.getElementById('qc-wastes-injection-actions-slot') : null;
  const actions = (
    <div className="flex items-center gap-2">
      <AgGridExportButton api={gridApi} fileName="plastic_wastes" variant="icon" />
...
export default PlasticWastesTable;
