...
import { Calendar, UserCircle2, Trash2, X, ChevronDown, ChevronUp, ChevronsUpDown, Download } from 'lucide-react';
import { useTasks } from './hooks/useTasks';
import { useStatusTranslation } from './hooks/useStatusTranslation';
import { useProjectMembers } from './hooks/useProjectMembers';
import { TaskDetailsModal } from './components/TaskDetailsModal';
import { StatusSelector } from './components/StatusSelector';
import { PrioritySelector, PriorityIcon } from './components/PrioritySelector';
...
export const TableView: React.FC<TableViewProps> = ({
  ...
  const { tasks, statuses, loading, updateTask, deleteTask, createTask, fetchStatuses } = useTasks(projectId);
  ...
  // Column sort state (overrides global sortBy when set)
  ...
  const toggleColSort = (key: ColKey) => {
    setColSort(prev => {
  ...
  const SortIcon: React.FC<{ colKey: ColKey }> = ({ colKey }) => {
    if (!colSort || colSort.key !== colKey) return <ChevronsUpDown className="w-3.5 h-3.5 text-gray-400 ml-1 shrink-0" />;
    return colSort.dir === 'asc'
      ? <ChevronUp className="w-3.5 h-3.5 text-blue-500 ml-1 shrink-0" />
      : <ChevronDown className="w-3.5 h-3.5 text-blue-500 ml-1 shrink-0" />;
  };
  ...
  React.useEffect(() => {
    if (createTrigger && createTrigger > 0) setShowCreateModal(true);
  }, [createTrigger]);
  ...
  const sortedTasks = useMemo(() => {
    ...
    return applyFilters([...filtered].sort((a, b) => {
      // Column sort takes priority over global sort
      if (colSort) {
        const { key, dir } = colSort;
        const mul = dir === 'asc' ? 1 : -1;
        switch (key) {
          case 'title': return mul * a.title.localeCompare(b.title);
          case 'status': return mul * (a.status_name || '').localeCompare(b.status_name || '');
          case 'priority': return mul * (priorityOrder[a.priority] - priorityOrder[b.priority]);
          case 'assignee': return mul * (a.assignee_name || '').localeCompare(b.assignee_name || '');
          case 'due_date': {
            if (!a.due_date && !b.due_date) return 0;
            if (!a.due_date) return mul;
            if (!b.due_date) return -mul;
            return mul * (new Date(a.due_date).getTime() - new Date(b.due_date).getTime());
          }
          case 'created_at':
  ...
  }, [tasks, statuses, sortBy, hideCompleted, filters, colSort]);
  ...
  const someSelected = allTaskIds.some(id => selectedTaskIds.has(id));
  ...
  if (loading && tasks.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-gray-400 text-sm">{t('listViewLoading')}</div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-white relative">
      <div className="flex-1 overflow-auto" style={{ paddingBottom: hasSelection ? '64px' : '0' }}>
            ...
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="w-10 px-3 py-2.5 border-r border-gray-200">
                <div
                  ...
                  style={{ borderColor: allSelected || someSelected ? '#2563eb' : '#d1d5db', backgroundColor: allSelected ? '#2563eb' : 'transparent' }}
                  onClick={toggleAll}
                >
                  {allSelected && <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                  {someSelected && !allSelected && <div className="w-1.5 h-0.5 bg-blue-500 rounded" />}
                </div>
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider min-w-[280px] border-r border-gray-200 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('title')}
              >
                <div className="flex items-center">{t('listViewColumnTask') || 'Task'}<SortIcon colKey="title" /></div>
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-40 border-r border-gray-200 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('status')}
              >
                <div className="flex items-center">{t('listViewColumnStatus') || 'Status'}<SortIcon colKey="status" /></div>
              </th>
              <th className="px-3 py-2.5 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-28 border-r border-gray-200">
                {t('tableMeta') || 'Meta'}
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-28 border-r border-gray-200 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('priority')}
              >
                <div className="flex items-center">{t('bulkPriority')}<SortIcon colKey="priority" /></div>
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-44 border-r border-gray-200 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('assignee')}
              >
                <div className="flex items-center">{t('bulkAssignee')}<SortIcon colKey="assignee" /></div>
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-40 border-r border-gray-200 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('due_date')}
              >
                <div className="flex items-center">{t('bulkDueDate')}<SortIcon colKey="due_date" /></div>
              </th>
              <th
                className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-24 cursor-pointer hover:bg-gray-100 select-none"
                onClick={() => toggleColSort('created_at')}
              >
                <div className="flex items-center">{t('tableCreated') || 'Created'}<SortIcon colKey="created_at" /></div>
              </th>
            </tr>
          </thead>
          <tbody>
            {sortedTasks.map((task) => {
              ...
              return (
                <tr
                  key={task.id}
                  className={`border-b border-gray-100 cursor-pointer group transition-colors ${isSelected ? 'bg-blue-50 hover:bg-blue-50' : 'hover:bg-gray-50'}`}
                  onClick={() => !hasSelection && setSelectedTask(task)}
                >
                  {/* Checkbox */}
                  <td className="px-3 py-2.5 border-r border-gray-100" onClick={(e) => e.stopPropagation()}>
                    <div
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer transition-all mx-auto ${hasSelection || isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                  ...
                  <td className="px-3 py-2.5 border-r border-gray-100">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {task.tags && task.tags.map((tag: any) => (
                        <span
                          key={tag.id}
                          className="hidden group-hover:inline-block text-xs px-1.5 py-0.5 rounded font-medium shrink-0"
                          style={{ backgroundColor: `${tag.color}18`, color: tag.color, border: `1px solid ${tag.color}35` }}
                        >
                          {tag.name}
                        </span>
                      ))}
                  ...
                  {/* Статус */}
                  <td
                    className="p-0 border-r border-gray-100 cursor-pointer"
                    onClick={(e) => { e.stopPropagation(); (e.currentTarget.querySelector('button') as HTMLButtonElement)?.click(); }}
                  >
                  ...
                  {/* Метки: подзадачи, комментарии, вложения */}
                  <td className="px-3 py-2.5 border-r border-gray-100">
                  ...
                  {/* Исполнитель — inline */}
                  <td
                    className="p-0 relative border-r border-gray-100 cursor-pointer"
                    onClick={(e) => { e.stopPropagation(); (e.currentTarget.querySelector('button') as HTMLButtonElement)?.click(); }}
                  ...
                  >
                    ...
                    <button
                      ...
                      className="group/assigneebtn flex items-center gap-1.5 rounded-md hover:bg-gray-100 px-1.5 py-1 transition-colors min-w-0 max-w-full"
                    >
                      {task.assignee_id && task.assignee_name ? (
                        ...
                        <>
                          ...
                          <span className="text-xs text-gray-300 group-hover/assigneebtn:text-gray-500 transition-colors">{t('taskModalAddAssignee')}</span>
                        </>
                      )}
                    </button>
                    </div>
                    {assigneeDropdownTaskId === task.id && (
                      <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-50 w-56">
                        ...
                        <div className="max-h-52 overflow-y-auto">
                          <button
                            onClick={() => { updateTask(task.id, { assignee_id: null }); setAssigneeDropdownTaskId(null); }}
                          ...
                          {members
                            ...
                            .map(m => (
                              <button
                                key={m.user_id}
                                onClick={() => { updateTask(task.id, { assignee_id: m.user_id }); setAssigneeDropdownTaskId(null); }}
                  ...
                  >
                    <div className="px-3 py-2.5">
                      <div className="group/datebtn flex items-center gap-1.5">
                        {task.due_date ? (
                          <>
                            <Calendar className={`w-4 h-4 shrink-0 ${new Date(task.due_date) < new Date() ? 'text-red-300' : 'text-gray-400'}`} />
                            <span className={`text-xs font-semibold ${new Date(task.due_date) < new Date() ? 'text-red-400' : 'text-gray-600'}`}>
                          ...
                          <>
                            ...
                            <span className="text-xs text-gray-300">{t('taskModalDueDate')}</span>
                          </>
                        )}
                      </div>
                    </div>
      ...
      {hasSelection && (
        <div className="absolute bottom-0 left-0 right-0 z-20 flex items-center gap-2 px-4 py-3 bg-white text-gray-700 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] border-t border-gray-200">
          ...
          <div className="relative" ref={bulkStatusRef}>
            ...
            {bulkStatusOpen && (
              <div className="absolute bottom-full mb-2 left-0 bg-white border border-gray-200 rounded-md shadow-xl z-30 w-48 py-1">
                {statuses.map(s => (
                  <button key={s.id} onClick={async () => { await bulkUpdate({ status_id: s.id }); setBulkStatusOpen(false); }}
            ...
            {bulkPriorityOpen && (
              <div className="absolute bottom-full mb-2 left-0 bg-white border border-gray-200 rounded-md shadow-xl z-30 w-40 py-1">
                {priorityConfig.map(p => (
                  <button key={p.value} onClick={async () => { await bulkUpdate({ priority: p.value }); setBulkPriorityOpen(false); }}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
          ...
          <div className="relative">
            ...
            <input ref={bulkDueDateRef} type="date"
              onChange={async (e) => { if (e.target.value) await bulkUpdate({ due_date: e.target.value }); e.target.value = ''; }}
              className="absolute bottom-0 left-0 w-0 h-0 opacity-0 border-0 p-0" tabIndex={-1} />
          </div>
          ...
          <div className="relative">
            ...
            <input ref={bulkStartDateRef} type="date"
          ...
          <div className="relative" ref={bulkAssigneeRef}>
            ...
            {bulkAssigneeOpen && (
              <div className="absolute bottom-full mb-2 left-0 bg-white border border-gray-200 rounded-md shadow-xl z-30 w-56">
                <div className="px-2 py-1.5 border-b border-gray-100">
                  <input autoFocus type="text" value={bulkAssigneeSearch}
                ...
                <div className="max-h-52 overflow-y-auto">
                  <button onClick={async () => { await bulkUpdate({ assignee_id: null }); setBulkAssigneeOpen(false); }}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-500 hover:bg-gray-50 transition-colors">
                    — {t('assigneeSelectorNotAssigned')}
                  </button>
                  {members.filter(m => (m.full_name || m.username || '').toLowerCase().includes(bulkAssigneeSearch.toLowerCase()))
                    .map(m => (
                      <button key={m.user_id} onClick={async () => { await bulkUpdate({ assignee_id: m.user_id }); setBulkAssigneeOpen(false); }}
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                        <Avatar name={m.full_name || m.username} imageUrl={`/avatar_${m.user_id}.png`} size="sm" />
          ...
          {canDelete && (
            <button onClick={bulkDeleteSelected} disabled={bulkProcessing}
      ...
      {selectedTask && (
        <TaskDetailsModal
          ...
          onUpdate={async (updates) => { await updateTask(selectedTask.id, updates); setSelectedTask(null); }}
