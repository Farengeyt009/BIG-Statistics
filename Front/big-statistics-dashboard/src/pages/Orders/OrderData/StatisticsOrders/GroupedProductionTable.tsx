...
const GroupedProductionTable: React.FC = () => {
  const { token } = useAuth();
  ...
  const { rowData, columnDefs, pinnedBottomRowData } = useMemo(() => {
    ...
    rawData.forEach(row => {
      ...
      const key = `${market}|${largeGroup}|${groupName}`;
    ...
    // Сортируем строки по убыванию Total (самые большие вверху)
    ...
    // 5. Генерируем колонки
    const cols: ColDef[] = [
      // Market - первый уровень группировки (скрыт)
      {
        field: 'Market',
        headerName: 'Market',
        rowGroup: true,
      ...
      // LargeGroup - второй уровень группировки (скрыт)
      {
        field: 'LargeGroup',
  ...
  // AutoGroupColumn для отображения Market, LargeGroup и GroupName
    ...
    cellRendererParams: {
      ...
      innerRenderer: (params: any) => {
        ...
        if (params.node?.group) {
          ...
          // Уровень 1 (LargeGroup) - без флага
      ...
      <div className="flex items-end justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-[#0d1c3d] mb-2">
            Uncompleted Orders by Market
          </h3>
          <div style={{ color: '#6b7280', fontSize: 14, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {t('statisticsTable.canBeUngrouped')} {t('statisticsTable.market')} 
        ...
        <div className="flex items-center gap-2">
          <AgGridGroupedExportButton api={gridApi} fileName="uncompleted_orders_by_market" variant="icon" />
        ...
        <>
          ...
          <div className="ag-theme-quartz" style={{ width: '100%' }}>
            ...
            initialGroupOrderComparator={(params) => {
              // Сортируем группы по убыванию aggData.Total
...
export default GroupedProductionTable;
