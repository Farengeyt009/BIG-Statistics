...
const PlanVsFactTable: React.FC<Props> = ({ selectedYear, leadTimeMonths }) => {
  ...
  const { rowData, columnDefs, pinnedBottomRowData } = useMemo(() => {
    ...
    // Объединяем план и факт в одну строку для каждой группы
    ...
    const cols: (ColDef | ColGroupDef)[] = [
      ...
      // LargeGroup - обычное поле, скрыто
      {
        field: 'LargeGroup',
  ...
  const autoGroupColumnDef = useMemo<ColDef>(() => ({
    ...
    cellRendererParams: {
      ...
      innerRenderer: (params: any) => {
        ...
        // Для leaf-строк показываем LargeGroup
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-semibold text-[#0d1c3d]">
            Sale Plan vs Actual Orders
          </h3>
          <AgGridGroupedExportButton api={gridApi} fileName="sale_plan_vs_actual_orders" variant="icon" leafNameField="LargeGroup" multiLevelHeader={true} />
        </div>
...
export default PlanVsFactTable;
