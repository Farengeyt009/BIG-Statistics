...
const SalePlanTableByGroup: React.FC<Props> = ({ selectedYear }) => {
  ...
  const { rowData, columnDefs, pinnedBottomRowData } = useMemo(() => {
    ...
    // Группируем данные по LargeGroup|Market (обратный порядок)
    const grouped: Record<string, ProcessedRow> = {};

    rawData.forEach(row => {
      // Нормализуем данные (trim + единый регистр для группировки)
      const marketRaw = (row.Market || 'Не указан').trim();
      const market = marketRaw;
      const largeGroupRaw = (row.LargeGroup || 'Не указано').trim();
      const largeGroup = largeGroupRaw;
      const monthIndex = row.MonthNum - 1; // 1-12 → 0-11
      const qty = row.QTY || 0;
      
      // Для ключа используем lowercase для объединения дубликатов
      const key = `${largeGroupRaw.toLowerCase()}|${marketRaw.toLowerCase()}`;
    ...
    const cols: ColDef[] = [
      ...
      // Market - обычное поле (не группируется), скрыто
      {
        field: 'Market',
        headerName: 'Market',
        rowGroup: false,
...
export default SalePlanTableByGroup;
