...
const WorkingSchedules: React.FC<WorkingSchedulesProps> = ({ isOpen, onClose, workCenters, canEdit }) => {
                    ...
                    <div className="space-y-6"> {/* ✅ Межкарточный интервал ~24-26px */}
                      {schedules.map((schedule, index) => (
                        <div
                          ...
                          className={`w-full h-[76px] px-4 py-3 rounded-2xl border transition-all duration-200 ${
                            canEdit ? 'cursor-pointer hover:border-opacity-80 hover:bg-opacity-95' : ''
                          } ${getCardColor(index)}`}
                          onClick={canEdit ? () => handleEditSchedule(schedule) : undefined}
                        >
                          {/* Сетка 2 колонки */}
