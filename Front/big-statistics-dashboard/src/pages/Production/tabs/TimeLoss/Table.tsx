...
const Table: React.FC<TableProps> = ({ date, startDate, endDate, selectedWorkShopIds, suppressLocalLoaders, onLoadingChange, isActive }) => {
  // Если пришел диапазон — передаем его в таблицу; иначе используем одну дату
  const currentDate = date || new Date().toISOString().split('T')[0];

  return (
    <div>
      <TimeLossTable date={currentDate} startDate={startDate} endDate={endDate} initialWorkShop={undefined} selectedWorkShopIds={selectedWorkShopIds} suppressLocalLoaders={suppressLocalLoaders} onLoadingChange={onLoadingChange} isActive={isActive} />
    </div>
  );
};
