...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  // цвета бэйджа
  const badgeText = useOrangeColor ? 'text-orange-600' : (changePercent === 0 ? 'text-orange-600' : (good ? 'text-green-700' : 'text-red-700'));
  const badgeBg   = useOrangeColor ? 'bg-orange-100' : (changePercent === 0 ? 'bg-orange-100' : (good ? 'bg-green-100' : 'bg-red-100'));
  ...
  return (
    <div className="flex flex-col justify-center min-w-[150px] first:pl-0 px-4">
      ...
      <div
        ...
        title={hideArrows ? `${changePercent.toLocaleString('ru-RU')}` : `${changePercent > 0 ? '+' : ''}${useRussianSeparator ? changePercent.toLocaleString('ru-RU') : changePercent}%`}
      >
        {/* arrow icon */}
        {!hideArrows && (
          <>
        ...
        <span>
          {hideArrows 
            ...
            : (useRussianSeparator ? Math.abs(changePercent).toLocaleString('ru-RU') : Math.abs(changePercent)) + '%'
          }
        </span>
      </div>
    </div>
